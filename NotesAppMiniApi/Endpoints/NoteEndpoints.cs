...
public static class NoteEndpoints
{
    public static void MapNoteEndpoints(this WebApplication app, List<User> users, List<Note> notes)
    {
        ...
        //1. Жоска хочу отобразить названия констант из енума NotesSort, но не понимаю как. Из-за этого сортировка в интерфейсе сваггера выглядит супер неинтуитивно.
        //Но возможно сама идея с enum была не очень, я хз
        //2. Также по SRP тут тоже какой-то пиздец, обработчик ищет юзера, фильтрует, сортирует. Но по условию задачи вроде можно было делать в рамках одного запроса
        //И как я понял, на одном route должен быть один get-запрос. Или нет?
        //3. Cтоит ли разгрузить этот запрос, создав методы поиска, сортировки, фильтрации в отдельных классах (условные NotesHandler, UsersHandler)?
            ...
            ExecutionPriority? executionPriority) =>
        {
            ...
            //Я не понял как в моём случае выдать ошибку доступа (Forbid), кроме как указать вручную.
